        ...
        for operation in self.config.operation_order:
            for operation_config in getattr(self.config, operation):
                ...
                if not operation_function:
                    raise ValueError(
                        f"Unable to match operation processor method \
...
class DataSetOperations(object):
    ...
    def calculate_percentage(self, data, source_column_names, result_column_name):
        ...
        data[result_column_name] = (data[numerator] / data[denominator] * 100).astype(
            float
        )

    def calculate_7_day_rolling_average(
        self, data, source_column_name, result_column_name
    ):
        data[result_column_name] = data[source_column_name].rolling(window=7).mean()
